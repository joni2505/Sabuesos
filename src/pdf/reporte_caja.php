...
$idcaja = $_GET['id'];
$rs = mysqli_query($conexion,"SELECT idcierre FROM cierre WHERE idcaja='$idcaja'");
...
<h4 class="font-weight-light my-4"><center>Detalle de Caja</center></h4>
        ...
        <?php 
         ...
         $rs = mysqli_query($conexion,"SELECT efectivo, transferencia, credito, debito, cuenta_corriente, venta_total, saldo_inicial  FROM cajadetalle WHERE idcierre='$idcierre'");
                ...
                <h1>Recaudacion de la Caja</h1>
                <h2>Efectivo: '.'$'.$total_efectivo.'</h2>
                <h2>Transferencia: '.'$'.$transferencia.'</h2>
                <h2>Tarjeta de Credito: '.'$'.$credito.'</h2>
                <h2>Tarjeta de Debito: '.'$'.$debito.'</h2>
                <h2>Cuenta Corriente: '.'$'.$cuenta.'</h2><br>

                <h1>Recaudacion Total</h1>
                <h2>Total: '.'$'.$venta_total.'</h2>
